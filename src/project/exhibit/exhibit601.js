import React from 'react';
import "./exhibit.css";
import fm1 from "../images/fm1.png";

function Exhibit601() {
    return (
        <div className="exhibit-column exhibit-col-light-blue">
            <div className="exhibit-links-container">
                <div className="exhibit-column exhibit-col-1">
                    <img src={fm1} className="exhibit-half-image" />
                </div>
                <div className="exhibit-column exhibit-col-1 exhibit-text-column">
                    <div className="label">
                        <span className="label-text">Name:</span>
                        <span className="value">Richard Feynman</span>
                    </div>
                    <div className="label">
                        <span className="label-text">Age in 2024:</span>
                        <span className="value">106 years</span>
                    </div>
                    <div className="label">
                        <span className="label-text">Roles:</span>
                        <span className="value">Physicist, Educator, Nobel Laureate</span>
                    </div>
                    <div className="label">
                        <span className="label-text">Key Highlights:</span>
                        <span className="value">
                            <ol>
                                <li>Richard Feynman won the Nobel Prize in Physics in 1965.</li>
                                <li>He made significant contributions to the development of quantum electrodynamics.</li>
                                <li>Feynman was known for his unique teaching style and popular science books.</li>
                            </ol>
                        </span>
                    </div>
                    <div className="label">
                        <span className="label-text">Great-Great... Influence on:</span>
                        <span className="value">
                            Quantum Mechanics, Particle Physics, Science Education, Computational Physics, Nanotechnology
                        </span>
                    </div>
                </div>
            </div>
        </div >
    );
}

export default Exhibit601;
